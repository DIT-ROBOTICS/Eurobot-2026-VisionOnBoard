...
sleep 1
tmux send-keys -t $SESSION_NAME:0 "export ROS_DOMAIN_ID=13 && source /opt/ros/humble/setup.bash && colcon build && source install/setup.bash && ros2 launch realsense2_camera rs_launch.py camera_namespace:=front camera_name:=front $CAMERA_SETTINGS" C-m
...
sleep 1
tmux send-keys -t $SESSION_NAME:0.1 "export ROS_DOMAIN_ID=13 && source /opt/ros/humble/setup.bash && source install/setup.bash && ros2 launch realsense2_camera rs_launch.py camera_namespace:=back camera_name:=back $CAMERA_SETTINGS" C-m

# Split pane 0 vertically for third camera (left)
tmux split-window -v -t $SESSION_NAME:0.0
tmux send-keys -t $SESSION_NAME:0.1 'docker exec -it vision-ws bash' C-m
sleep 1
tmux send-keys -t $SESSION_NAME:0.1 "export ROS_DOMAIN_ID=13 && source /opt/ros/humble/setup.bash && source install/setup.bash && ros2 launch realsense2_camera rs_launch.py camera_namespace:=left camera_name:=left $CAMERA_SETTINGS" C-m
...
sleep 1
tmux send-keys -t $SESSION_NAME:0.3 "export ROS_DOMAIN_ID=13 && source /opt/ros/humble/setup.bash && source install/setup.bash && ros2 launch realsense2_camera rs_launch.py camera_namespace:=right camera_name:=right $CAMERA_SETTINGS" C-m
