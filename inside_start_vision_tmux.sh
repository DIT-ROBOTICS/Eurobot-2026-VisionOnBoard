...
tmux send-keys -t ${SESSION_NAME}:${WIN_ID}.0 "export ROS_DOMAIN_ID=\${ROS_DOMAIN_ID:-13} && source /opt/ros/humble/setup.bash && colcon build && source install/setup.bash && ros2 launch realsense2_camera rs_multi_camera_launch.py" C-m
...
ros2 launch aruco_object scanner_multi.launch.py team_color:=\${MODE:-blue}
'
tmux send-keys -t ${SESSION_NAME}:${WIN_ID}.2 "$SCANNER_CMD" C-m

# Pane 3 (Bottom Right): Shell / Utils
tmux send-keys -t ${SESSION_NAME}:${WIN_ID}.3 'export ROS_DOMAIN_ID=${ROS_DOMAIN_ID:-13} && source /opt/ros/humble/setup.bash && source install/setup.bash' C-m
...
# launch object feedback
ros2 launch object_feedback obj_fb_bringup.launch.py
